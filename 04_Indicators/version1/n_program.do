	...
	foreach var in `varlist' {
       ...
       set matsize 5000
		...
		preserve
			...
			collapse (count) `var' [aw=weight_`ds'], by(dist_key year)
	   ...
	   // Create the final matrix
		...
		save "$pe/count/district/`ds'/`tv'.dta", replace
		restore
        ...
        ********************************************************************* 
		...
		preserve
			...
			collapse (count) `var' [aw=weight_`ds'], by(province year)
	   ...
	   // Estimate the SE
		...
		save "$pe/count/province/`ds'/`tv'.dta", replace
		restore
        ...
        ********************************************************************* 
		...
		preserve
			...
			collapse (count) `var' [aw=weight_`ds'], by(country year)
	   ...
	   // Estimate the SE
		...
		save "$pe/count/country/`ds'/`tv'.dta", replace
		restore
