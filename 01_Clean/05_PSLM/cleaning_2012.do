...
**** Step 1: Get Raw Data Files & check for unique identifiers: 
		...
		use "$pslm_raw/2012/weight.dta"
	
		merge 1:m psu using "$pslm_raw/2012/plist_1.dta"
		keep if _merge ==3
		drop _merge 
...
**** Step 2: Check administrative data vars and create a clean dist_key:
	    ...
	    * replace dist_nm = "" if province !=2
	    * replace dist_nm = "RAWALPINDI" if dist_nm == "ISLAMABAD"

		
		* Dist_key:
		gen dist_key= . if prov ==1
		replace dist_key = 1 if dist_nm == "�TTOCK"	& prov ==1	 
		replace dist_key = 2 if dist_nm == "BHAWANAGAR" & prov ==1
		replace dist_key = 3 if dist_nm == "BAHAWALPUR" & prov ==1
		replace dist_key = 4 if dist_nm == "BHAKAR" & prov ==1
		...
		replace dist_key = 10 if dist_nm == "HAFAIZABAD"  & prov ==1
		...
		replace dist_key = 18 if dist_nm == "LODHREAN"  & prov ==1
		replace dist_key = 19 if dist_nm == "MANDI BAHAUDDIN"  & prov ==1
		replace dist_key = 20 if dist_nm == "M IANWALI"  & prov ==1
		...
		replace dist_key = 25 if dist_nm == "PAKPATAN"  & prov ==1
		...
		* Attock as a special characater not recognised by stata in the string:
		
	    replace dist_key = 1 if dist_key == . & prov ==1	
		...
		*KP:
		...
		replace dist_key = 54 if dist_nm == "NOWSEHRA" & prov ==4
		...
		* Sindh:
	    ...
	    replace dist_key = 62 if dist_nm == "BADDIN" & prov ==2 
	    ...
	    replace dist_key = 72  if dist_nm == "MITIARI" & prov ==2		 
	    ...
	    replace dist_key = 78 if dist_nm == "SHIOKARPUR" & prov ==2
	    ...
	    replace dist_key = 82 if dist_nm == "TANDO MOHD KHAN" & prov ==2
	    ...
	    * Dist_key for BALOCHISTAN:
	    ...
	    replace dist_key = 93 if dist_nm == "GAWADAR" & prov ==3
	    ...
	    replace dist_key = 103 if dist_nm == "LASBELLA" & prov ==3
	    ...
	    replace dist_key = 108 if dist_nm == "NAUSKI" & prov ==3
	    replace dist_key = 109 if dist_nm == "PASHIN" & prov ==3
	    replace dist_key = 110 if dist_nm == "QUETTA" & prov ==3
	    replace dist_key = 111 if dist_nm == "SHEANI" & prov ==3
	    ...
	    replace dist_key = 118 if dist_nm == "KEYCH/TURBAT" & prov ==3
	    ...
	    replace dist_key = 200 if dist_nm == "NAWABSHA" & prov ==2
...
**** Step 3: Merge Education data with administrative data & clean/harmonize variables:
		...
		* HH ROSTER
		...
		merge 1:1 hhcode idc using "$pslm_raw/2012/roster.dta"
		drop _merge 

		
		* EDUC
		
		merge 1:1 hhcode idc using "$pslm_raw/2012/sec_c.dta"
		ren _merge _merge1
...
**** Step 4: Merge other data sections: 
		...
		* Get income var:
		merge 1:1 hhcode idc using"$pslm_raw/2012\sec_e.dta"	
		drop _merge
	
		*Get asset vars
		merge m:1 hhcode using"$pslm_raw/2012\sec_f2.dta"
		drop _merge
		
		*Other asset vars
		merge m:1 hhcode using"$pslm_raw/2012\sec_g.dta"
		drop _merge
		...
		use "$pslm_raw/2012\sec_f1.dta", clear
		keep if itc==1
...
**** Step 5: Create relevant vars for the panel:
	...
	****************************************************************************
	    ...
	    * Var cleaning:
	    ...
	    recode sf2q11a sf2q11b sf2q11c sf2q11d sf2q11e sf2q11f sf2q11g sf2q11h sf2q11i sf2q11j sf2q11k sf2q11l ///
		sf2q11m sf2q11n sf2q11o sf2q11p sf2q11q sf2q11r sf2q11s sf2q11t (2=0)
	   ...
	   * cooking range   
		...
		* washing machine
		...
		quiet: estpost sum WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone WI_agriculture_land WI_iron  ///
		WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr WI_refrigerator WI_air_cooler WI_ac WI_computer WI_bicycle ///
		WI_motorcycle WI_car WI_tractor WI_mobile WI_cooking_range WI_stove WI_washingmachine
		...
		*	Check and drop blank observations:
...
**** Step 6: Save clean cross-sectional dataset:
		...
		save "$pslm_clean/PSLM_clean_2012.dta", replace 
