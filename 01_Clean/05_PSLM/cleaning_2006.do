...
**** Step 1: Get Raw Data Files & check for unique identifiers: 
		...
		use "$pslm_raw/2006/section b with weights.dta"	, clear
...
**** Step 2: Check administrative data vars and create a clean dist_key:
		...
		* cleaning district name:
	    * replace dist_nm = "" if province !=1
	    * replace dist_nm = "RAWALPINDI" if dist_nm == "ISLAMABAD"
		
		* Dist_key:
		
		gen dist_key = . if prov ==1
		...
		replace dist_key = 26 if dist_nm == "RAHIMYAR KHAN" & prov ==1
...
**** Step 3: Merge Education data with administrative data & clean/harmonize variables:
	    ...
	    * EDUC
		merge 1:1 hhcode idc using "$pslm_raw/2006/section c.dta"
...
**** Step 4: Merge other data sections: 
		...
		* Get income var:
		merge 1:1 hhcode idc using "$pslm_raw/2006/section e.dta"	
		drop _merge

		*Get asset vars
		merge m:1 hhcode using"$pslm_raw/2006/section f2.dta"
		drop _merge
     
	    * Other assets:		
		merge m:1 hhcode using"$pslm_raw/2006/section g.dta"
		drop _merge 
		...
		use "$pslm_raw/2006/section f1.dta", clear
		ren q1to10 itc
		keep if itc==1
...
**** Step 5: Create relevant vars for the panel:
	...
	****************************************************************************
	  ...
	  * Var cleaning:
	  ...
	  recode sf2q11a sf2q11b sf2q11c sf2q11d sf2q11e sf2q11f sf2q11g sf2q11h sf2q11i sf2q11j sf2q11k sf2q11l sf2q11m sf2q11n sf2q11o sf2q11p (2=0)
	 ...
	 * assets missing: sf2q11u sf2q11v sf2q11w 	sf2q11x 
      ...
      * Tractor
	  ...
	  quiet: estpost sum WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone WI_agriculture_land ///
	  WI_iron WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr WI_refrigerator WI_air_cooler WI_ac ///
	  WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor 
	  ...
	  * SES 1: taking Yearly total income variable
	  pca WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone WI_agriculture_land WI_iron ///
	  WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr WI_refrigerator WI_air_cooler ///
	  WI_ac WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor 
	
	  predict Asset_Level

	  alpha WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone WI_agriculture_land ///
...
**** Step 6: Save clean cross-sectional dataset:
      ...
      save "$pslm_clean/PSLM_clean_2006.dta", replace 
