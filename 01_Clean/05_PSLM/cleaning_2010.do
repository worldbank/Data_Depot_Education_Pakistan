...
**** Step 1: Get Raw Data Files & check for unique identifiers: 
		...
		use "$pslm_raw/2010/weights.dta", clear
	
		merge 1:m hhcode using "$pslm_raw/2010/sec_b_dist.dta"
		drop _merge 	
...
**** Step 2: Check administrative data vars and create a clean dist_key:
		...
		* Dist_key:
		...
		replace dist_key = 4 if dist_nm == "BHAKHAR" & prov ==1 
		...
		* KP:
		...
		replace dist_key = 52 if dist_nm == "MANSHERA" & prov ==4
		...
		* Sindh:
	    ...
	    replace dist_key = 66 if dist_nm == "JACCOBABAD" & prov ==2
	    ...
	    replace dist_key = 72  if dist_nm == "MAITARI" & prov ==2
		 
	    replace dist_key = 73 if dist_nm == "MIR PUR KHAS" & prov ==2
	    ...
	    replace dist_key = 82 if dist_nm == "TANDO MUDA KHAN" & prov ==2
	    ...
	    replace dist_key = 200 if dist_nm == "NAWABSHAH" & prov ==2
		...
		* Dist_key for BALOCHISTAN:
	    ...
	    replace dist_key = 90 if dist_nm == "BOLAN/KACCHI" & prov ==3
	    replace dist_key = 91 if dist_nm == "CHAGI" & prov ==3
	    ...
	    replace dist_key = 100 if dist_nm == "QILLAH ABDULLAH" & prov ==3
	    replace dist_key = 101 if dist_nm == "QILLAH SAIFUALLH" & prov ==3
...
**** Step 3: Merge Education data with administrative data & clean/harmonize variables:
		...
		* EDUC
	    ...
	    merge 1:1 hhcode idc using "$pslm_raw/2010/sec_c_dist.dta"
	    ren _merge _merge1
		...
		* fix highest_grade 
		...
		replace highest_grade = 14 if highest_grade ==16
		...
		* fix current_grade 
		...
		replace current_grade = 14 if current_grade ==16
...
**** Step 4: Merge other data sections: 
	   ...
	   * Addtional code to create quintiles (to be incorporated in master cleaning file)
		...
		* Get income var:
		merge 1:1 hhcode idc using"$pslm_raw/2010/sec_e_dist.dta"	
		drop _merge


		*Get asset vars
		merge m:1 hhcode using"$pslm_raw/2010/sec_f2_dis.dta"
		drop _merge

		* Other asset vars
		
		merge m:1 hhcode using"$pslm_raw/2010/sec_g_dis.dta"
		drop _merge
		...
		use "$pslm_raw/2010/sec_f1_dis.dta", clear
		ren q1to10 itc
		keep if itc==1
...
**** Step 5: Create relevant vars for the panel:
	   ...
	   * parent's education:
		...
		*note: foreing & remittances , rent and income from all other sources not avaiable
		...
		egen year_income = rsum (yr_seq08 seq10 seq15 seq17 seq19 seq21), missing
		...
		* agriculture land:
		 ...
		 * Var cleaning:
		 ...
		 recode sf2q11a	sf2q11b sf2q11c sf2q11d sf2q11e sf2q11f sf2q11g sf2q11h sf2q11i sf2q11j sf2q11k sf2q11l sf2q11m	sf2q11n	sf2q11o	sf2q11p ///
		 sf2q11q sf2q11r sf2q11s sf2q11t (2=0)
		...
		* washing machine
		...
		WI_ac WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor WI_mobile WI_cooking_range WI_stove WI_washingmachine
		...
		WI_air_cooler WI_ac WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor WI_mobile WI_cooking_range WI_stove WI_washingmachine
		...
		* Drop and check blank observations:
		
		drop if dist_key ==.
		
**** Step 6: Save clean cross-sectional dataset:
		
		save "$pslm_clean/PSLM_clean_2010.dta", replace 
