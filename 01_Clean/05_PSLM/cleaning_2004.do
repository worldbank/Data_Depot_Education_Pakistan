...
**** Step 1: Get Raw Data Files & check for unique identifiers: 
		...
		use "$pslm_raw/2004/sec-c.dta", clear
		duplicates tag hhcode idc, gen(x)
        ta x
		drop if x ==1
		
		/*
		Note_2004: unique identifiers are not unique for 2 obs - cleaning issue	
		*/
		
		
		tempfile educ_2004
		save `educ_2004', replace
			
	
	    use "$pslm_raw/2004/sec-b.dta", clear 
...
**** Step 2: Check administrative data vars and create a clean dist_key:
	    ...
	    gen prov_AR = "KP" if  province ==3
		replace prov_AR = "PUNJAB" if province ==1
		replace prov_AR = "SINDH" if province ==2
		...
		* Dist_key:
		...
		replace dist_key = 10 if dist_nm == "HAFIZA ABAD" & prov ==1 
		replace dist_key = 11 if dist_nm == "JEHLAM" & prov ==1 
		...
		replace dist_key = 17 if dist_nm == "LEIAH" & prov ==1  
		...
		replace dist_key = 26 if dist_nm == "R.Y.KHAN" & prov ==1 
		...
		replace dist_key = 30 if dist_nm == "SARGHODHA" & prov ==1 
		replace dist_key = 31 if dist_nm == "SHEILUPURA" & prov ==1 
		replace dist_key = 32 if dist_nm == "SIALKOT" & prov ==1 
		replace dist_key = 33 if dist_nm == "T.T.SINGHL" & prov ==1 
		...
		replace prov = 1 if dist_nm == "T.T.SINGHL"
		replace dist_key = 33 if dist_nm == "T.T.SINGHL" & prov ==1 
		...
		*KP:
		...
		replace dist_key = 39 if dist_nm == "BATGRAM" & prov ==4
		replace dist_key = 40 if dist_nm == "BONAIR" & prov ==4
		replace dist_key = 41 if dist_nm == "CHARSADA" & prov ==4
		...
		replace dist_key = 46 if dist_nm == "KARK" & prov ==4
		...
		replace dist_key = 60 if dist_nm == "TORGARH" & prov ==4
		...
		replace dist_nm = "KARACHI" if dist_nm == "DISTRICT OF KARACHI"
		...
		replace dist_key = 73 if dist_nm == "MIRPUR KHA" & prov ==2
		replace dist_key = 74 if dist_nm == "NEHERO FEROZ" & prov ==2
		...
		replace dist_key = 82 if dist_nm == "TANDO MIHD KHAN" & prov ==2
		replace dist_key = 83 if dist_nm == "THAEPARKAR" & prov ==2
		...
		replace dist_key = 200 if dist_nm == "NAWAB SHAH" & prov ==2
		...
		* Dist_key for BALOCHISTAN:
		...
		replace dist_key = 90 if dist_nm == "BOLAN /KARACHI" & prov ==3
		replace dist_key = 91 if dist_nm == "CHAGHI" & prov ==3
		replace dist_key = 92 if dist_nm == "DERA BUGHTI" & prov ==3
	    replace dist_key = 93 if dist_nm == "GWADAR" & prov ==3 
		replace dist_key = 94 if dist_nm == "HARNAI" & prov ==3
		replace dist_key = 95 if dist_nm == "JAFERABAD" & prov ==3
		...
		replace dist_key = 101 if dist_nm == "QILLAH SIAFULLAH" & prov ==3
		replace dist_key = 102 if dist_nm == "KOHLU" & prov ==3
		replace dist_key = 103 if dist_nm == "LASBILLA" & prov ==3
		replace dist_key = 104 if dist_nm == "LORALI" & prov ==3
		...
		replace dist_key = 108 if dist_nm == "NUSKI" & prov ==3
		replace dist_key = 109 if dist_nm == "PESHIN" & prov ==3
		...
		replace dist_key = 118 if dist_nm == "KETCH/TURBAT" & prov ==3
		replace dist_key = 119 if dist_nm == "PUNJGUR" & prov ==3	 
...
**** Step 3: Merge Education data with administrative data & clean/harmonize variables:
		...
		merge 1:1 hhcode idc using `educ_2004'
		ren _merge _merge1
	
		****************************
		
		ren sbq03 relation_hh_head
		ren sbq01 gender
        ren sbq04 age
	    ...
	    * checking in_school vars with grades
		fre in_school_ever
		fre highest_grade  
		...
		* fix current_grade 
		replace current_grade = 20 if current_grade ==19
		replace current_grade = 14 if current_grade ==17
		replace current_grade = 17 if current_grade ==16
		replace current_grade = 21 if current_grade ==15
		replace current_grade = 16 if current_grade ==14
		replace current_grade = 15 if current_grade == 13
...
**** Step 4: Merge other data sections: 
		...
		* Addtional code to create quintiles (to be incorporated in master cleaning file)
		
		destring psu, replace
		* Get income var:
		merge 1:1 hhcode idc using "$pslm_raw/2004/section e_new.dta"	
		drop _merge

		*Get HH asset vars
		merge m:1 hhcode using"$pslm_raw/2004/secf  all households.dta"
		drop _merge
		...
		* Other assets var
	    merge m:1 hhcode using"$pslm_raw/2004/sec-g.dta"
		drop _merge


	
	    ************************************************************************
        * dropping missing idcs
		drop if idc ==.	
		isid hhcode idc		   	
        ************************************************************************		
...
**** Step 5: Create relevant vars for the panel:
	  ...
	  *  HH Head:  Highest Education:
		...
		* recode others as missing 
	    ...
	    ren highest_grade hg_head
		
		
		* keeping the highest grade for hh head:
		
	    bysort hhcode: egen highest_grade_head = max(hg_head)
		
		duplicates tag hhcode, gen (x)
		egen rank = rank( highest_grade_head), by(hhcode)
		...
		la val highest_grade_head scq04
		
	
		drop hg_head members rank x
	...
	**************************************************	
	   ...
	   * parent's education:
	    ...
	    ************************************************************************
		...
		replace hh_educ_status = 6 if hh_es == 15 | hh_es == 16 |hh_es == 17 | hh_es == 18 // degrees   
		...
		* monthly income:
		 gen yr_seq08 = seq13 * seq14
		 
	    * Total yearly income (including remittances (domestic & foreign),
		
		*note: foreing & remittances , rent and income from all other sources not avaiable
		
		egen year_income = rsum (yr_seq08 seq16), missing
		...
		*/
		 ...
		 * Var cleaning:
		 recode sf2q11a	sf2q11b sf2q11c sf2q11d sf2q11e sf2q11f sf2q11g sf2q11h ///
		 sf2q11i sf2q11j sf2q11k sf2q11l sf2q11m sf2q11n sf2q11o sf2q11p (2=0)
		 

		* assets missing: sf2q11u sf2q11v sf2q11w 	sf2q11x 
	 ...
	 * motorcycle
	  ...
	  * Tractor
	   ...
	   //Final Income Variable for PCA analysis to create a Wealth Index
	    quiet: estpost sum  WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone ///
	   WI_iron WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr WI_refrigerator ///
	   WI_air_cooler WI_ac WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor 
		...
		* SES 1: taking Yearly total income variable
	    pca  WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone  ///
	    WI_iron WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr ///
	    WI_refrigerator WI_air_cooler WI_ac WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor 

	    predict Asset_Level
	
	    alpha  WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone ///
	    WI_iron WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr ///
	    WI_refrigerator WI_air_cooler WI_ac WI_computer WI_bicycle WI_motorcycle WI_car WI_tractor 
...
**** Step 6: Save clean cross-sectional dataset:
     	...
     	save "$pslm_clean/PSLM_clean_2004.dta", replace 	
