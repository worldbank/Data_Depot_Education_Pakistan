...
**** Step 1: Get Raw Data Files & check for unique identifiers: 
	   ...
	   use "$pslm_raw/2008/weights_file.dta", clear
		
       merge 1:m psu using "$pslm_raw/2008/sec_b.dta"
	   drop _merge 
...
**** Step 2: Check administrative data vars and create a clean dist_key:
       ...
       la def prov 1 "punjab" 2 "sindh" 3 "khyber pakhtunkhwa" 4 "balochistan"
	   ...
	   * Dist_key:
	   ...
	   replace dist_key = 31 if dist_nm == "SHEIKUPURA" & prov ==1
...
**** Step 3: Merge Education data with administrative data & clean/harmonize variables:
		...
		* EDUC
		...
		merge 1:1 hhcode idc using "$pslm_raw/2008/sec_c.dta"
		ren _merge _merge1
...
**** Step 4: Merge other data sections: 
		...
		* Get income var:
		merge 1:1 hhcode idc using "$pslm_raw/2008/section_e.dta"	
		drop _merge

		*Get asset vars
		merge m:1 hhcode using"$pslm_raw/2008/section_f2.dta"
		drop _merge

		* Other asset var
		
		*Get asset vars
		merge m:1 hhcode using"$pslm_raw/2008/section_g.dta"
		drop _merge
		...
		use "$pslm_raw/2008/section_f1.dta", clear
		ren q1to10 itc
		keep if itc==1
	    ...
	    ************************************************************************
	    * drop 17 blank observations with missing idcs
	    drop if idc ==.
        ************************************************************************	
	...
	****************************************************************************
     ...
     ***************************************************************************
		...
		* Tractor
		...
		WI_iron WI_fan WI_sewing_machine WI_radio WI_tablechair WI_clock WI_tv WI_vcr WI_refrigerator WI_air_cooler ///
		...
		* SES 1: taking Yearly total income variable
		pca WI_energytype_light WI_toilet WI_occupancy WI_energytype_light WI_phone WI_agriculture_land /// 
        ...
        * Drop for missing dist_key - check bkank obs	
		drop if dist_key ==.
...
**** Step 6: Save clean cross-sectional dataset:
		...
		save "$pslm_clean/PSLM_clean_2008.dta", replace 
